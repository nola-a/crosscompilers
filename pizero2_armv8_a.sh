...
git clone -b rpi-6.6.y --single-branch  --depth=1 https://github.com/raspberrypi/linux
...
../binutils-$BINUTILS/configure --prefix=$CROSSC --target=arm-linux-gnueabihf  --with-arch=armv8-a --with-fpu=neon-fp-armv8 --with-float-abi=hard --disable-multilib
make -j 8
make install
cd ..
...
../gcc-$GCC/configure --prefix=$CROSSC --target=arm-linux-gnueabihf --enable-languages=c,c++,fortran  --with-arch=armv8-a --with-fpu=neon-fp-armv8 --with-float-abi=hard --disable-multilib
make -j8 all-gcc
make install-gcc
...
../glibc-$GLIBC/configure --prefix=$CROSSC/arm-linux-gnueabihf --build=$MACHTYPE --host=arm-linux-gnueabihf --target=arm-linux-gnueabihf  --with-arch=armv8-a --with-fpu=neon-fp-armv8 --with-float-abi=hard --with-headers=$CROSSC/arm-linux-gnueabihf/include --disable-multilib libc_cv_forced_unwind=yes --disable-werror
